...
class SimAnneal(object):
    ...
    def __init__(self, coords, T = -1, alpha = -1, stopping_T = -1, stopping_iter = -1):
        ...
        print('stopping_iter', stopping_iter)
        self.stopping_temperature = 0.0000000001 #if stopping_T == -1 else stopping_T
    ...
    def visualizeRotes(self):
        ...
        x = self.coords[self.best_solution, 0]
        y = self.coords[self.best_solution, 1]
        plt.plot(x, y)

        plt.plot(self.coords[:, 0], self.coords[:, 1], 'ro')
        plt.title('Simulated Annealing Algorithm: Bays29, Cost: {}, Runtime: {} seconds'.format(self.best_fitness,time.time() - self.start_time))
        # plt.title('Runtime of Simulated annealing: {}'.f)
        plt.show()
        # visualize_tsp.visualize(self.best_solution, self.coords, self.best_fitness)
